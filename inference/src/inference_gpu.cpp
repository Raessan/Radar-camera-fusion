...
#include <opencv2/cudawarping.hpp> // To resize GpuMat
#include <opencv2/cudaarithm.hpp> // To normalize GpuMat
#include <set>
#include <random>
#include <memory>
#include <chrono>
...
cv::cuda::GpuMat ones_mat(cv::Size(w_nn_radarcam, h_nn_radarcam), CV_32FC1);
...
cv::Mat do_inference(cv::Mat image,const Eigen::Matrix<float, 3, Eigen::Dynamic> &radar){
    ...
    if (n_points > 0){
        ...
        nn_handler_depth_anything->run_inference(input_nn_depth_anything, output_nn_depth_anything, false, true);

        // Get the output in cv::Mat format
        cv::cuda::GpuMat im_output_depth_anything(h_depth_anything, w_depth_anything, CV_32FC1, output_nn_depth_anything);

        // Now we start preparing the input to the radar-cam NN. It requires a resize and normalization
        cv::cuda::GpuMat im_input_radarcam;
        // Resized image
        cv::cuda::resize(im_output_depth_anything, im_input_radarcam, cv::Size(w_nn_radarcam, h_nn_radarcam), cv::INTER_LINEAR);
        cv::cuda::normalize(im_input_radarcam, im_input_radarcam, 0, 1, cv::NORM_MINMAX, CV_32FC1);
        // This step is necessary because Depth Anything outputs inverses of depth
        //im_input_radarcam = 1.0f - im_input_radarcam;
        cv::cuda::subtract(ones_mat, im_input_radarcam, im_input_radarcam);

        cv::Mat matrix_test;
        im_input_radarcam.download(matrix_test);

        // Input of 2nd NN
        //input_radarcam[0][0] = im_input_radarcam.ptr<float>();

        // ALERT. AS OPPOSED TO cv::Mat, cv::cuda::GpuMat AREN'T USUALLY CONTINUOUS. ONLY THEIR ROWS ARE!! FOR THAT REASON, WE HAVE TO PASS THE DATA ROW BY ROW
        //std::cout << "Is the GPU matrix continuous?: " << im_input_radarcam.isContinuous() << std::endl;
        // ITERATE OVER ROWS TO OVERCOME THE PROBLEM OF CONTINUITY
        for (int i=0; i<h_nn_radarcam; i++)
            checkCuda(cudaMemcpy(&input_radarcam[0][0][i*w_nn_radarcam], im_input_radarcam.ptr<float>(i), sizeof(float)*w_nn_radarcam, cudaMemcpyDeviceToDevice));
        checkCuda(cudaMemcpy(input_radarcam[1][0], radar_adapted.data(), sizeof(float)*3*nn_radar_points, cudaMemcpyHostToDevice));
        ...
        // Run inference. The data is on the GPU (from_device=true)
        nn_handler_radarcam->run_inference(input_radarcam, output_radarcam, true);
...
int main(){
    ...
    // Matrix of 1s on the GPU
    ones_mat.setTo(cv::Scalar(1.0f));

    // Adapt input for the 2nd network. We need to allocate data on the GPU for the second input (the first input is already on the GPU)
    input_radarcam[0].resize(1);
    input_radarcam[1].resize(1);
    checkCuda(cudaMalloc((void**)&input_radarcam[0][0], sizeof(float)*w_nn_radarcam*h_nn_radarcam));
    checkCuda(cudaMalloc((void**)&input_radarcam[1][0], sizeof(float)*3*nn_radar_points));
    ...
    checkCuda(cudaFree(input_radarcam[0][0]));
    checkCuda(cudaFree(input_radarcam[1][0]));
